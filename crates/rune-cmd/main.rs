...
fn init_tracing(env: &PlatformEnv) -> Result<()> {
    ...
    let fmt_layer: Box<dyn Layer<tracing_subscriber::Registry> + Send + Sync> =
        if env.log_format_json {
            ...
            Box::new(tracing_subscriber::fmt::layer().with_filter(suppress_hyper))
        };
...
use cli::{AgentCommand, ArtifactCommand, Cli, ClusterCommand, Command, DaemonCommand};
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Command::Chat(args) => commands::chat::exec(args).await?,
