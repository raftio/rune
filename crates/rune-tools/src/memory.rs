...
pub async fn list(
    ...
    let rows: Vec<(String,)> = sqlx::query_as("SELECT key FROM rune_kv ORDER BY key")
        .fetch_all(&ctx.db)
        .await
        .map_err(|e| format!("Database error: {e}"))?;
...
pub async fn recall(
    ...
    let row: Option<(String,)> = sqlx::query_as("SELECT value FROM rune_kv WHERE key = ?")
        .bind(key)
    ...
    match row {
        ...
        None => Ok(
            serde_json::json!({ "key": key, "value": null, "message": format!("No value found for key '{key}'.") }),
        ),
    }
}
